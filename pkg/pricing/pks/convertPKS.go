...
func getPKSRateCard(pararegion string) *Pricing {
	...
	for _, specificRegion := range pksPricing {
		if specificRegion.Region == pararegion {
			fmt.Println("getPKSRateCard", specificRegion)
...
func GetRateCardForPKS(region string) *models.RateCard {
	pksRateCard := getPKSRateCard(region)
	fmt.Println("getRateCardForPKS : ", pksRateCard)
	return getPurserRateCard(region, pksRateCard)
}

// One Diffrence between array and declaration
