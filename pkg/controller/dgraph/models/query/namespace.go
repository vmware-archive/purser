...
func RetrieveNamespaceHierarchy(name string) JSONDataWrapper {
	...
	query := getQueryForHierarchy("isNamespace", "namespace", name, "@filter(has(isDeployment) OR has(isStatefulset) OR has(isJob) OR has(isDaemonset) OR (has(isReplicaset) AND (NOT has(deployment))))")
...
func RetrieveNamespaceMetrics(name string) JSONDataWrapper {
	...
	query := getQueryForNamespaceMetrics(name)
	return getJSONDataFromQuery(query)
}
